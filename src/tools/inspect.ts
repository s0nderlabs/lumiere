...
export function registerInspect(server: McpServer): void {
  server.tool(
    "inspect",
    "Get metadata about a video (local file path or URL: YouTube, X/Twitter, Vimeo, direct mp4, etc.) without processing it. Returns duration, resolution, codec, fps, audio presence. Also returns per-tier context-cost estimate so the caller knows how much context the next watch() call will burn, plus a recommended tier and an autocompact warning if applicable. Routes through yt-dlp for URLs. Pass `exact_tokens=true` to extract one probe frame per tier and call Anthropic's /v1/messages/count_tokens for exact conversation-token estimates (requires LUMIERE_ANTHROPIC_API_KEY; adds ~1-2s). NOTE: this and the other lumiere MCP tools are the PERCEPTION half; to CREATE a launch video, use the /lumiere skill's creation flow (not an MCP tool).",
    {
