...
7. **Export to mp4 (final step, only once a version is chosen).** Render via the stable entry point: `bun $CLAUDE_PLUGIN_ROOT/bin/lumiere-render.mjs <project-dir>`. Report the exact command first. Output lands at the lock's `meta.render.output` else `renders/<project>.mp4`; fps from `meta.render.fps` else 30; override with `--out <file>` / `--fps <n>`. ENGINE CHOICE: `hyperframes` (DEFAULT) mixes the lock's audio into the mp4. `--engine own` is lumiere's frame-exact pipeline (single-thread); `--engine own-parallel` shards it across N workers (`--shards`, default 4, tuned for a 16GB box) for fast 4K. BOTH own engines are VIDEO-ONLY (no audio mix), so for a lock with `status:locked` audio either render with hyperframes OR render own/own-parallel then mux: `ffmpeg -i master.mp4 -i <audio> -map 0:v:0 -map 1:a:0 -c:v copy -c:a aac -b:a 384k final.mp4` (add `-shortest` only to clip the longer stream; omit it to let a video tail hold past a shorter audio bed). EMBEDDED VIDEO: a composition that embeds a per-frame-seeked `<video>` (a full-bleed demo cut) MUST use `--engine own-parallel` - hyperframes serves over http so a `file://` video src won't load and it has no seeked-wait (it tears); own-parallel loads `file://` and waits for each frame's `seeked` (see RESTAGE.md "Embedded <video>"). The Preview pane's "Copy render command" button emits this for the loaded composition.
